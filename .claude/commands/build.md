---
description: Implement a previously planned work item from docs/roadmap.md — reads its plan file, is plan-mode-aware, branches, and builds it to the Definition of Done. The build counterpart to /interview's stop-after-plans mode.
...
# Context
This project plans work up front with `/interview` (which can stop after writing plans) and records it in `docs/roadmap.md`, with the full plan in `docs/plan/<ID>.md`. `/build` picks up one such item and implements it on its own branch. The plan is the contract — you are **not** re-running the interview.
...
Using build v1 by Josué Clément
...
- **If I passed a work-item ID as the argument** (`$ARGUMENTS`), that is the target. A base ID (`FEATURE-001`) resolves to the item's next phase whose status is `TODO` or `IN PROGRESS` (for a multi-phase item), or to the item itself (for a single-phase one); a phase ID (`FEATURE-001-PHASE02`) targets that specific phase. If the ID isn't in the roadmap, stop and tell me — don't guess.
...
3. Verify the skill's preconditions: the working tree is clean and the base branch is known (confirm it — don't assume `main`). If the tree is dirty, or a branch with the target name already exists, stop and ask.

## Phase 4: Build
Follow the `dev-workflow` skill's **build flow**:
1. Create the dev's branch from the base branch (`feature/…` for a feature, `bugfix/…` for a bug, per the skill), then flip the item/phase status to `IN PROGRESS` as the first change on the branch.
...
3. Meet the **Definition of Done**: the build is clean, the whole test suite passes (including the new tests the plan's acceptance criteria call for), every acceptance criterion is met, and the roadmap + plan statuses are updated. Never report an item `DONE` with a failing build or tests — report the failure instead.
